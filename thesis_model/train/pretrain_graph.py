...
from data_pipeline.convert_smiles_to_pyg import ATOM_VOCAB_SIZES
from data_pipeline.data import load_graph_dataset
from thesis_model.model.encoders import GraphEncoder
from common.wandb_utils import add_wandb_args, wandb_finish, wandb_init, wandb_log
...
def _augment_view(graph):
    ...
    if getattr(view, "edge_index", None) is not None and view.edge_index.numel() > 0:
        ...
        # D5: edge_attr rows must stay aligned with the edges they describe --
        # dropping edge_index columns without dropping the matching edge_attr
        # rows silently mislabels every remaining edge's bond features.
        if getattr(view, "edge_attr", None) is not None and view.edge_attr.numel() > 0:
            view.edge_attr = view.edge_attr[mask]
    # D5: x is now a LongTensor of categorical field indices (atomic_num, degree, ...),
    # not a continuous feature vector -- adding Gaussian noise to a category index is
    # meaningless (and torch.randn_like errors on an integer dtype anyway). Edge
    # dropout alone is still a standard, valid graph-contrastive augmentation
    # (same family as GraphCL's edge perturbation); node noise is just dropped.
    return view
...
class GraphContrastivePretrainer:
    ...
    def train_epoch(self, loader: DataLoader, on_batch_end=None) -> Dict[str, float]:
        ...
        for step, (view_1, view_2) in enumerate(loader, start=1):
            ...
            _, states_1 = self.encoder(view_1.x, view_1.edge_index, view_1.batch, edge_attr=getattr(view_1, "edge_attr", None))
            _, states_2 = self.encoder(view_2.x, view_2.edge_index, view_2.batch, edge_attr=getattr(view_2, "edge_attr", None))
...
def _mask_node_features(x: torch.Tensor, mask_prob: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Zero out mask_prob of node rows (index 0 doubles as a BERT-style [MASK] category --
    no real RDKit atomic_num is 0). Returns (masked_x, mask, original_values_at_mask)."""
    num_nodes = x.size(0)
...
class GraphMultiTaskPretrainer:
    def __init__(
        ...
        atom_vocab_sizes: Optional[List[int]] = None,
    ) -> None:
        self.encoder = encoder.to(device)
        self.device = torch.device(device)
        ...
        # D5: x's fields are categorical indices, not continuous magnitudes, so
        # masked-attribute prediction is classification (one head + cross-entropy
        # per field), not the old single-head MSE regression against raw indices --
        # that made the exact same "index-as-magnitude" mistake this refactor fixes
        # everywhere else.
        atom_vocab_sizes = list(atom_vocab_sizes) if atom_vocab_sizes else list(ATOM_VOCAB_SIZES)
        self.atom_vocab_sizes = atom_vocab_sizes
        self.mask_heads = nn.ModuleList([nn.Linear(hidden_dim, size) for size in atom_vocab_sizes]).to(self.device)
    ...
    def train_epoch(self, loader: DataLoader, on_batch_end=None) -> Dict[str, float]:
        ...
        for step, (batch, desc_labels, fg_labels) in enumerate(loader, start=1):
            ...
            node_state, _ = self.encoder(masked_x, batch.edge_index, batch.batch, edge_attr=getattr(batch, "edge_attr", None))
            pooled = global_mean_pool(node_state, batch.batch)

            masked_hidden = node_state[mask]
            mask_loss = sum(
                self.mask_criterion(head(masked_hidden), original[:, field_idx])
                for field_idx, head in enumerate(self.mask_heads)
            ) / len(self.mask_heads)
...
if __name__ == "__main__":
    ...
    import thesis_model.train.pretrain_graph as _self
