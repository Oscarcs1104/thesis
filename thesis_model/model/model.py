...
from thesis_model.model.encoders import VALID_GRAPH_BACKBONES, GraphEncoder, LanguageEncoder
    ...
    def _get_states(self, data) -> Tuple[Optional[torch.Tensor], torch.Tensor]:
        ...
        if self.use_graph:
            _, layer_graph_states = self.graph_encoder(
                data.x, data.edge_index, batch, edge_attr=getattr(data, "edge_attr", None)
            )
