...
_POOL_FNS = {"add": global_add_pool, "mean": global_mean_pool, "max": global_max_pool}
...
class GraphEncoder(nn.Module):
    ...
    and the pooled graph vector after every layer.

    Argument order is ``(x, edge_index, batch, edge_attr=None)`` -- edge_attr last and
    optional -- so callers that predate bond features (and crossmodal_model's
    HybridEncoder) keep working unchanged.
    """
    ...
    def __init__(
        ...
        **_ignored: Any,  # absorbs removed node_encoding / node_vocab_sizes / edge_vocab_sizes kwargs
        ...
        self.pool = pool
        # mean_max concatenates two hidden_dim pooled vectors then projects back down, so
        # every downstream consumer keeps seeing exactly hidden_dim from the graph branch.
