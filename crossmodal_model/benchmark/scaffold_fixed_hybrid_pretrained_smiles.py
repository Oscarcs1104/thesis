"""HybridMoLA-PretrainedSMILES benchmark: same protocol/seeds as scaffold_fixed_hybrid.py
(official DeepChem split), SMILES branch swapped for a pretrained transformer (ChemBERTa,
frozen by default) instead of MoLA's from-scratch char-level branch -- see
crossmodal_model/model/mola_hybrid_pretrained_smiles.py.

Usage:
    python crossmodal_model/benchmark/scaffold_fixed_hybrid_pretrained_smiles.py
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="HybridMoLA with a pretrained (ChemBERTa) SMILES branch")
    ...
    args.config_name = "NewTestHybridPretrainedSMILES"
    if args.out is None:
...
def build_datasets(dataset_name: str):
    ...
    train_data, valid_data, test_data = _load("train"), _load("valid"), _load("test")
...
def run_one_seed(dataset_name, train_data, valid_data, test_data, seed: int, args) -> Dict[str, float]:
...
def main() -> None:
    ...
    with out_path.open(mode, newline="", encoding="utf-8") as handle:
        ...
        for dataset_name in args.datasets:
            ...
            train_data, valid_data, test_data = build_datasets(dataset_name)
            per_seed: List[Dict[str, float]] = []
            for seed in args.seeds:
                start = time.time()
                metrics = run_one_seed(dataset_name, train_data, valid_data, test_data, seed, args)
