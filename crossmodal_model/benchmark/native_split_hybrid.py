"""MoLA-Hybrid on the re-derived native split (scaffold or random) -- reads the exact same
per-seed split dumps crossmodal_model/benchmark/native_split.py produces under
data/rederived_splits/<splitter>/<dataset>/seed_<seed>/, so MoLA G+S, thesis_model's own
native_split.py, and this one all train on identical molecules. Does NOT re-derive the
split itself -- run native_split.py --splitter <scaffold|random> first.

Usage:
    python crossmodal_model/benchmark/native_split_hybrid.py --splitter random
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="MoLA-Hybrid on the re-derived native split (scaffold or random)")
    ...
    args.config_name = f"NewTestHybrid_{args.splitter}split"
    if args.out is None:
...
def load_dump(dataset_name: str, seed: int, splitter: str, cfg: Dict[str, str]):
    ...
    def _load(p):
        df = pd.read_csv(p)
        return df["smiles"].astype(str).tolist(), df[cfg["target_col"]].astype(float).tolist()
...
def run_one(dataset_name: str, seed: int, args) -> Dict[str, float]:
    ...
    (train_smiles, train_y), (valid_smiles, valid_y), (test_smiles, test_y) = load_dump(dataset_name, seed, args.splitter, cfg)
    vocab = build_vocab(train_smiles + valid_smiles + test_smiles)

    train_data = prepare_hybrid_data(train_smiles, train_y, vocab, max_sm_len=args.max_sm_len)
    valid_data = prepare_hybrid_data(valid_smiles, valid_y, vocab, max_sm_len=args.max_sm_len)
    test_data = prepare_hybrid_data(test_smiles, test_y, vocab, max_sm_len=args.max_sm_len)
    ...
    model = HybridMoLA(
        ...
        num_layers=args.num_layers, graph_backbone=args.graph_backbone, graph_pooling=args.graph_pooling,
    ).to(device)
    ...
    def train_epoch() -> float:
        ...
        for batch in train_loader:
            ...
            preds = _forward(batch)
            targets = standardizer.transform(batch.y.float().view_as(preds))
    ...
    def eval_epoch(loader) -> Dict[str, float]:
        model.eval()
        total, n = 0.0, 0
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                targets = standardizer.transform(targets_orig)
                loss = criterion(preds, targets)
                preds_all.append(standardizer.inverse_transform(preds).cpu())
                targets_all.append(targets_orig.cpu())
                total += loss.item() * batch.num_graphs
                n += batch.num_graphs
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_metrics = eval_epoch(valid_loader)
